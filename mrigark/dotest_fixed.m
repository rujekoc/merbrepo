function [err_max,err_rms,max_rate,rms_rate,time,nffast,nfslow] = dotest_fixed(co,Gammas,Bi,fs,ff,tvals,Y0,h,mvals,Ytrue)
  % USAGE:[err_max,err_rms,max_rate,rms_rate,time,nffast,nfslow] = dotest_fixed(co,Gammas,Bi,fs,ff,tvals,Y0,h,mvals,Ytrue)
  % Calls solve_MRIGARKERK in the fixed linearization setting and outputs
  % results
  %
  ...
  for i=1:length(mvals)
    ...
    for j=1:length(h)
      ...
      % March over tvals
      tstart = tic;
      [tout,Y,ns,nf,ierr] = solve_MRIGARKERK(fs,ff,tvals,Y0,co,Gammas,Bi,hs,hf);
      telapsed = toc(tstart);
      time(i,j) = telapsed;
