% driver_bicoupling_merk_dynamic
% Set up for dynamic linearization
...
Jn = @bicoupling_nonauto_Jacobian;
fn = @bicoupling_nonauto;
Nn = @bicoupling_nonauto_nonlinearity_gen; % nonlinearity generating function

% time parameters
n       = 21;
tout    = linspace(t0,tf,n);                         % immediate times for solution
hmax    = tout(2)-tout(1);                           % largest time step
h       = hmax*0.5.^(0:7);                           % large time step
...
stepsolvers = {'ERK-3-3', 'ERK-4-4', 'ARK5(4)8L[2]SA-ERK'};
stagesolvers = stepsolvers;
...
for isol = 1:length(solvers)
  ...
  % Print out details of current solve
  ...
  for j = 1:length(h)
    ...
    for k = 2:n
      ...
      Y0step     = Y(:,k-1);
      [Yout,fastcalls,slowcalls] = solver(A,fs,rkstage,rkstep,Y0step,m,[tout(k-1),tout(k)],hs,Nn,Jn);
      Y(:,k) = Yout;
