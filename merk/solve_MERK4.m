function [u_n,nffast,nfslow] = solve_MERK4(A,gn,internalsolver,finalstepsolver,u0,m,tvals,h,Nn,JacFn)
  % Usage: [u_n,nffast,nfslow] = solve_MERK4(A,gn,internalsolver,finalstepsolver,u0,m,tvals,h,Nn,JacFn)
  %
  % Fourth order multirate exponential Runge Kutta solver for the  ODE problem
  %     u' = F(t, u(t)) = A*u + gn(t,u(t)), t in tvals,
  %     u(t0) = u0
  %
  % Inputs:
  %     A              = Matrix corresponding to linear portion
  %     gn             =  string holding function name for nonlinear portion
  ...
  %     Nn             = string holding function name for nonlinearity generating function
  %     JacFn          =  string holding function name for Jacobian
  %
  % Outputs:
  ...
  if (nargin == 9)
    error('Total number of inputs is 8 or 10')
    return;
  end
